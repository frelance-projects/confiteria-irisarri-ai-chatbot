...
export async function sendAutoTagWhatsapp(message, assistants, data) {
  //SS BAILEYS
  if (provider.whatsapp.provider === 'baileys') {
    for (const assistant of assistants) {
      if (assistant.whatsappId) {
        if (!assistant.whatsappId) {
          console.info('sendAutoTagWhatsapp: no hay id de whatsapp para el asistente', assistant)
          continue
        }
        addToQueue(
...
async function sendMessages({ message, assistant }) {
  ...
  if (res) {
    ...
    sendLog(
      'error',
      'tools/autoTag/functions/send/sendAutoTagWhatsapp',
      'sendMessages',
