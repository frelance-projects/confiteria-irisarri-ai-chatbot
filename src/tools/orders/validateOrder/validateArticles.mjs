...
export async function validateArticles(articles) {
  ...
  for (const item of articles) {
    ...
    if (typeof item.quantity !== 'number' || item.quantity <= 0) {
      console.error(`Cantidad del artículo con código ${item.code} no es un número positivo.`)
      itemErrors.push(`Cantidad del artículo con código ${item.code} no es un número positivo.`)
    }
    ...
    const baseArticle = await getArticleByCode(item.code)
    if (!baseArticle) {
      console.error(`Artículo con código ${item.code} no encontrado.`)
      errors.push(`Artículo con código ${item.code} no encontrado.`)
    }
    ...
    const dailyArticle = await getDailyArticleByCode(item.code)
    if (dailyArticle) {
