...
export async function sendAppointmentWhatsappMeta(template, assistants, parameters) {
  for (const assistant of assistants) {
    if (!assistant.whatsappId) {
      console.info('sendAppointmentWhatsappMeta: no hay id de whatsapp para el asistente', assistant)
      continue
    }
    addToQueue(
      {
...
async function sendTemplate({ template, assistant, parameters }) {
  ...
  if (res) {
    ...
    sendLog(
      'error',
      'tools/appointment/functions/send/sendAppointmentWhatsappMeta',
      'sendTemplate',
