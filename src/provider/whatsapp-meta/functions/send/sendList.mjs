 ...
 */
export async function sendList(phone, { header, body, footer, buttonText }, list) {
  try {
    // Validar entradas
    ...
    const data = {
      ...
      interactive: {
        ...
        action: {
          ...
          sections: [
            {
              title: list.title || 'Elige una opción',
    ...
    if (body) data.interactive.body = { text: body }
    ...
    if (error.response) {
      ...
      console.error('Error al enviar el mensaje interactivo:', error.message)
    }
    return null
  }
}
