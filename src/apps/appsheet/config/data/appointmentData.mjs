...
export async function getAppointmentDataByAppointmentId(appointmentId) {
  const appointmentData = await getTable(appsheeTablesData.toolAppointmentCalendarData, [], {
    Selector: `FILTER(${appsheeTablesData.toolAppointmentCalendarData}, [TOOL_APPOINTMENT_CALENDAR] = "${appointmentId}")`,
    Locale: 'en-GB',
    Timezone: ENV.TZ,
...
export async function addAppointmentData(appointmentData) {
  ...
  const res = await postTable(appsheeTablesData.toolAppointmentCalendarData, data)
  if (res) {
